...
public abstract class AbstractAggregationFunctionTest {
  ...
  public class WithSegment {
    ...
    public QueryExecuted whenQuery(String query) {
      ...
      try {
        ...
        ServerQueryRequest serverQueryRequest = new ServerQueryRequest(instanceRequest, _serverMetrics, System.currentTimeMillis());
        InstanceResponseBlock responseBlock = _queryExecutor.execute(serverQueryRequest, QUERY_RUNNERS);
  ...
  public static class QueryExecuted {
    ...
    public QueryExecuted thenResultIs(Object[]... expectedResult) {
      List<Object[]> expectedList = Arrays.asList(expectedResult);
      Assert.assertEquals(_responseBlock.getResultsBlock().getRows(), expectedList);
      return this;
    }
  }
