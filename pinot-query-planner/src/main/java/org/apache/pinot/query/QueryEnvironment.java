...
public class QueryEnvironment {
   ...
   */
  private static HepProgram getOptProgram(PinotRuleSet ruleSet, Set<String> skipRuleSet,
    ...
    List<RelOptRule> basicRules = filterRuleList(
        ruleSet.rulesFor(Phase.BASIC), skipRuleSet, useRuleSet, defaultDisabledRuleSet);
    List<RelOptRule> filterPushdownRules = filterRuleList(
        ruleSet.rulesFor(Phase.FILTER_PUSHDOWN), skipRuleSet, useRuleSet, defaultDisabledRuleSet);
    List<RelOptRule> projectPushdownRules = filterRuleList(
        ruleSet.rulesFor(Phase.PROJECT_PUSHDOWN), skipRuleSet, useRuleSet, defaultDisabledRuleSet);
    List<RelOptRule> pruneRules = filterRuleList(
        ruleSet.rulesFor(Phase.PRUNE), skipRuleSet, useRuleSet, defaultDisabledRuleSet);
  ...
  private static HepProgram getTraitProgram(@Nullable WorkerManager workerManager, Config config,
    ...
    if (!usePhysicalOptimizer) {
      // POST_LOGICAL: per-query copy because we may need to swap the PinotSortExchangeCopyRule
      // with one configured for the per-query sortExchangeCopyLimit threshold.
      List<RelOptRule> postLogical = new ArrayList<>(ruleSet.rulesFor(Phase.POST_LOGICAL));
      // Per-query sortExchangeCopyLimit override: if it differs from the rule's configured threshold,
      // build a parameterized rule and replace the configured one in the per-query list.
      if (sortExchangeCopyLimit != PinotSortExchangeCopyRule.SORT_EXCHANGE_COPY.config.getFetchLimitThreshold()) {
        PinotSortExchangeCopyRule overridden = ImmutablePinotSortExchangeCopyRule.Config.builder()
            ...
            .build()
            .toRule();
        for (int i = 0; i < postLogical.size(); i++) {
          if (postLogical.get(i) instanceof PinotSortExchangeCopyRule) {
            postLogical.set(i, overridden);
            break;
          }
        }
      }
    ...
    if (!usePhysicalOptimizer) {
      // apply RelDistribution trait to all nodes — these rules depend on the per-query
      // WorkerManager (for PinotImplicitTableHintRule), so they stay outside PinotRuleSet.
  ...
  public interface Config {
    ...
    /**
     * Broker's {@link PinotRuleSet} — the singleton service that owns the per-phase
     * rule lists after plugin {@code RuleSetCustomizer}s have run. Callers without a
     * Guice injector (tests, non-broker roles) receive the OSS defaults via the
     * {@link DefaultRuleSetCustomizer}.
     */
    @Value.Default
    default PinotRuleSet getRuleSet() {
      return new PinotRuleSet(Set.of(new DefaultRuleSetCustomizer(new PinotConfiguration())));
    }
