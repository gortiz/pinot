...
public enum Phase {
  ...
  /// project pushdown phase). OSS defaults: `DefaultRuleSetCustomizer.FILTER_PUSHDOWN_RULES`.
  FILTER_PUSHDOWN,

  /// Project pushdown rules. OSS defaults: `DefaultRuleSetCustomizer.PROJECT_PUSHDOWN_RULES`.
  PROJECT_PUSHDOWN,

  /// Top-down pruning rules. OSS defaults: `DefaultRuleSetCustomizer.PRUNE_RULES`.
  PRUNE,

  /// Post-logical rules used when the physical optimizer is **not** enabled.
  /// OSS defaults: `DefaultRuleSetCustomizer.postLogicalRules(sortExchangeCopyLimit)`.
  /// The list contains a `PinotSortExchangeCopyRule` configured with the broker
  /// default `sortExchangeCopyLimit`; per-query overrides of that limit are
  /// applied by `QueryEnvironment` swapping the rule on a per-query copy of
  /// this list.
  POST_LOGICAL,
