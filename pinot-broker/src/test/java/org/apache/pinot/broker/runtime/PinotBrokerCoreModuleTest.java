...
public class PinotBrokerCoreModuleTest {
  ...
  public void ossDefaultsSeedEveryPhase() {
    ...
    assertTrue(ruleSet.rulesFor(Phase.BASIC).size() > 0, "BASIC should be populated");
    assertTrue(ruleSet.rulesFor(Phase.FILTER_PUSHDOWN).size() > 0, "FILTER_PUSHDOWN should be populated");
    assertTrue(ruleSet.rulesFor(Phase.PROJECT_PUSHDOWN).size() > 0, "PROJECT_PUSHDOWN should be populated");
    assertTrue(ruleSet.rulesFor(Phase.PRUNE).size() > 0, "PRUNE should be populated");
    assertTrue(ruleSet.rulesFor(Phase.POST_LOGICAL).size() > 0, "POST_LOGICAL should be populated");
    assertTrue(ruleSet.rulesFor(Phase.POST_LOGICAL_V2).size() > 0, "POST_LOGICAL_V2 should be populated");
  ...
  public void pluginCustomizerCanRemoveOssDefault() {
    Injector baseInjector = Guice.createInjector(new PinotBrokerCoreModule(new PinotConfiguration()));
    PinotRuleSet baseRuleSet = baseInjector.getInstance(PinotRuleSet.class);
    String firstRuleName = baseRuleSet.rulesFor(Phase.BASIC).get(0).toString();
    int ossSize = baseRuleSet.rulesFor(Phase.BASIC).size();
