...
public class MicrometerMetricsRegistry implements PinotMetricsRegistry {
  ...
  public void removeMetric(PinotMetricName name) {
    ...
    _gaugeHolders.remove(metricName);
    // A name may map to more than one meter (e.g. once tag-first naming lands); remove them all.
    for (Meter meter : _composite.find(metricName).meters()) {
  ...
  public <T> PinotGauge<T> newGauge(PinotMetricName name, @Nullable final PinotGauge<T> gauge) {
    String metricName = name.getMetricName().toString();
    // computeIfAbsent registers the gauge exactly once per name and retains a strong reference to the holder, so the
    // exported gauge never decays to NaN and a re-registration returns a wrapper over the canonical (exported) holder.
    MicrometerSettableGauge<T> holder = (MicrometerSettableGauge<T>) _gaugeHolders.computeIfAbsent(metricName, k -> {
      ...
      Gauge.builder(metricName, newHolder, h -> toDouble(h.getValue())).strongReference(true).register(_composite);
      return newHolder;
    });
  ...
  public PinotMeter newMeter(PinotMetricName name, String eventType, TimeUnit unit) {
    String metricName = name.getMetricName().toString();
    Counter counter = Counter.builder(metricName).register(_composite);
