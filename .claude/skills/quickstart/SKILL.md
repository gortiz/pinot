...
2. **If the script is missing, offer to build.** Ask the user:
   ...
   > - Just pinot-tools (Maven 4): `mvnd2/mvnd/mvn4 -pl pinot-tools package -DskipTests` (~3 min)
   > - Just pinot-tools (Maven 3): `./mvnw -pl pinot-tools -am package -DskipTests` (~3 min)
   > (Prefer `mvnd2` → `mvnd` → `mvn4` → `./mvnw` in that order. See CLAUDE.md "Build tool selection".)
