 ...
 * under the License.
 */
package org.apache.pinot.udf.test.test.scenarios;
...
import org.apache.pinot.udf.test.test.PinotFunctionTestCluster;
import org.apache.pinot.udf.test.test.UdfTestResult;
...
public class PredicateUdfTestScenario extends AbstractUdfTestScenario {
  public PredicateUdfTestScenario(PinotFunctionTestCluster cluster, boolean nullHandlingEnabled) {
  ...
  @Override
  public Map<UdfExample, UdfTestResult> execute(Udf udf, UdfSignature signature) {
    ...
    String sql = replaceCommonVariables(udf, signature, isNullHandlingEnabled(), ""
            ...
            + "  )\n");
    PinotFunctionTestCluster.ExecutionContext context = new PinotFunctionTestCluster.ExecutionContext(
        isNullHandlingEnabled(),
        false);
    Map<UdfExample, UdfTestResult> queryResult = extractResultsByCase(udf, signature, _cluster.query(context, sql));

    Set<UdfExample> successfulCases = queryResult.values()
        .stream()
        .map(UdfTestResult::getTest)
        .collect(Collectors.toSet());

    Set<UdfExample> examples = udf.getExamples().get(signature);
    Map<UdfExample, UdfTestResult> result = new HashMap<>();
    ...
    for (UdfExample expectedPositive : expectedPositives) {
      result.put(expectedPositive, UdfTestResult.success(udf, expectedPositive, true, true));
    }
    ...
    if (expectedPositives.size() != udf.getExamples().size()) {
      ...
      for (UdfExample unexpected : unexpectedNegatives) {
        result.put(unexpected, UdfTestResult.success(udf, unexpected, false, true));
      }
      ...
      for (UdfExample unexpected : unexpectedPositives) {
        result.put(unexpected, UdfTestResult.success(udf, unexpected, true, false));
      }
    }
    return result;
  }
