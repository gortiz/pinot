...
public class PinotFunctionEnvGenerator {
  ...
  public static void prepareEnvironment(PinotFunctionTestCluster cluster, Udf udf) {
    Schema schema = generateSchema(udf);
    cluster.addTable(schema, generateTableConfig(udf));

    cluster.addRows(
        PinotFunctionEnvGenerator.getTableName(udf),
        schema,
        udf.getExamples().entrySet().stream().flatMap(entry -> {
  ...
  public static String getTableName(Udf udf) {
    return "table_" + udf.getMainFunctionName().replaceAll("[^a-zA-Z0-9]", "_");
  }
  ...
  public static String getParameterColumnName(UdfParameter param, int argIndex) {
    return "arg" + argIndex + "_" + param.getDataType().name().toLowerCase(Locale.US);
  }
  ...
  public static String getResultColumnName(UdfParameter param, boolean nullHandling) {
    String nullHandlingSuffix = nullHandling ? "_nullable" : "";
    return "result" + nullHandlingSuffix + "_" + param.getDataType().name().toLowerCase(Locale.US);
  }

  public static TableConfig generateTableConfig(Udf udf) {
    TableConfigBuilder tableConfigBuilder = new TableConfigBuilder(TableType.OFFLINE)
        .setTableName(getTableName(udf));
  ...
  public static Schema generateSchema(Udf udf) {
    Schema.SchemaBuilder schemaBuilder = new Schema.SchemaBuilder();
    schemaBuilder.setSchemaName(PinotFunctionEnvGenerator.getTableName(udf));
