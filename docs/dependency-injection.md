...
| `PinotRuleSet` | same package | `@Singleton` service, takes `Set<RuleSetCustomizer>`, applies them in iteration order, freezes per-phase lists |
...
| `QueryEnvironment.Config` | `pinot-query-planner/.../query/` | `PinotRuleSet getRuleSet()` accessor with `@Value.Default` that builds from `DefaultRuleSetCustomizer`; `getOptProgram` / `getTraitProgram` read every phase from it |
...
| --------------------- | ---------------------------------------------------------------------------------------- |
| `pinot-spi`           | Nothing (Guice-free; sees `getPluginClassLoaders()` only).                                |
